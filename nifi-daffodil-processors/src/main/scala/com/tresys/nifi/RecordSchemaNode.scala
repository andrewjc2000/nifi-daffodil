...
class RecordSchemaNode(dsomElement: TermView) {
    ...
    case element: ElementDeclView =>
      if (element.isSimpleType) {
        val recordFieldType: RecordFieldType = element.simpleType.primType match {
          ...
          case _ => throw new UnsupportedOperationException(s"unsupported type for element $element")
        }
  ...
  // not really necessary because children is public, but makes the code more readable
  // Potential Improvement: see if there is a better way to modify a List that is known to dynamically change
  def addChild(newChild: RecordSchemaNode): Unit = children = children :+ newChild
