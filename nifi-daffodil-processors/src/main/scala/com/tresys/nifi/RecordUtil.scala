...
object RecordUtil {
  ...
  def log(logger: ComponentLog, message: String, args: Array[Object]): Unit = {
    if (!PRODUCTION_MODE) logger.error(message, args)
  }

  def log(logger: ComponentLog, message: String): Unit = log(logger, message, new Array[Object](0))
  ...
  def recordListAsString(javaList: util.List[RecordField], tabs: String): String = {
  ...
  def dataTypeListAsString(javaList: util.List[DataType], tabs: String): String = {
